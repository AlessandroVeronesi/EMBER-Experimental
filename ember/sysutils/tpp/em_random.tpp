...
//
template <typename T>
T ember::math::random::random(const size_t& bitwidth) requires(std::integral<T>) {
...
//
template <typename T>
T ember::math::random::uniform(const T& start, const T& stop) requires(std::integral<T>) {
    if (start == stop) {
        return start;
    }
    static std::random_device rd;
    static std::mt19937 gen(rd());
    std::uniform_int_distribution<T> dis(start, stop - 1);
